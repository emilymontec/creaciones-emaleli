...
const KANBAN_COLUMNS: { estado: string; label: string; color: string }[] = [
  ...
  { estado: "EN_REVISION", label: "En revisión", color: "border-amber-500 bg-amber-50/30" },
  { estado: "EN_PRODUCCION", label: "En producción", color: "border-purple-500 bg-purple-50/30" },
  { estado: "EMPACADO", label: "Empacado", color: "border-indigo-500 bg-indigo-50/30" },
  { estado: "ENVIADO", label: "Enviados", color: "border-blue-500 bg-blue-50/30" },
  { estado: "ENTREGADO", label: "Entregados", color: "border-emerald-500 bg-emerald-50/30" },
];
          ...
          >
            ...
            <div className="flex-1 space-y-3">
              {colPedidos.length === 0 ? (
                ...
                colPedidos.map((p) => (
                  <Card key={p.id} className="p-3 shadow-xs hover:shadow-card transition-shadow">
                    ...
                    <p className="text-xs font-medium text-gray-800 truncate">{p.cliente?.nombre ?? "—"}</p>
                    <p className="text-[11px] text-gray-400 truncate">{p.ciudad}</p>

                    <div className="mt-2.5 flex items-center justify-between border-t border-gray-100 pt-2 text-[11px]">
                      <span className="font-semibold text-gray-900">${Number(p.total).toLocaleString("es-CO")}</span>
                      {Number(p.saldoPendiente) > 0 ? (
                        <span className="text-amber-700 font-medium">Saldo: ${Number(p.saldoPendiente).toLocaleString("es-CO")}</span>
                      ) : (
